...
(gmail-ui/contract/api.schema.json). This fails when the API's models drift from that snapshot, which
...
def test_the_committed_schema_matches_the_apis_models():
    ...
    assert committed == build_schema(), (
        ...
        "`python -m scripts.export_addon_schema`, then update gmail-ui/src/types.ts and the add-on tests."
    )

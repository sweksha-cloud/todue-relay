...
ToDue Relay reads one Gmail inbox on an hourly schedule, decides which emails describe a deadline
or an action item (an RSVP, a scheduling request, "rent is due the 1st"), extracts structured data
from them with an LLM, and either creates a Google Calendar event directly (when it is confident
and the date looks real) or holds the item for a person to review. Two surfaces let a person make
that call: a server-rendered web dashboard, and a Gmail Workspace Add-on that shows the same data
and the same actions inside Gmail itself. Every decision the system makes is recorded in Postgres,
so a crash or a re-run never reprocesses an email or creates a duplicate event.

### The pieces

**1. Fetch — `app/gmail_client.py`.** Calls the Gmail API for unread, recent messages. Detects two
things that should never be independently extracted: a real calendar invite (a `.ics` part, MIME
type `text/calendar`/`application/ics`) and an email Google Calendar's own notification system sent
(`calendar-notification@google.com`, checked against both the `Sender` and `From` headers — see
decision 6 and the Trash-button decision, 28). `trash_message()` is the module's one write
operation, moving a message to Gmail's Trash; every other function only reads.

**2. Pre-filter — `app/filters.py`.** A cheap keyword/pattern scoring pass that runs before any LLM
call, so an obvious non-candidate never spends Gemini quota. `scripts/tune_filter.py` measures its
real-world pass rate against a live inbox.

**3. Extract — `app/llm_client.py`, `app/schemas.py`.** Sends the pre-filtered candidates to the
Gemini API with a strict response schema (a Pydantic model: event name, deadline text, a *parsed*
date, confidence, action type, recurrence, source context). A response that doesn't validate is
treated as a failure for that one email, never the whole batch.

**4. Parse and judge the date — `app/date_utils.py`.** Turns the LLM's free-text date into a real,
timezone-aware `datetime` (`parse_deadline_date`), resolving US timezone abbreviations explicitly
rather than trusting `dateutil`'s ambiguous guess (`EST` is always -5:00, never DST-adjusted), and
judges whether a technically-parseable date is *plausible* (`is_plausible`, decision 10) —
separately from whether it parses at all. `parse_local_wallclock` does the reverse: turns a
person-picked date/time string (typed into either surface) into the same kind of timezone-aware
value.

**5. Route — `app/pipeline.py`.** Confidence routing (decision 9): high confidence with a plausible
date creates a Calendar event immediately, a recurring one if the email describes a repeating
obligation; everything else (low confidence, an implausible date, or no fixed date at all) goes to
the review queue or the Action Items list instead of being guessed at. Also does duplicate/reschedule
matching — the same deadline seen twice, or moved — by name similarity plus reschedule wording.

**6. Sync to Calendar — `app/calendar_client.py`.** Wraps the Google Calendar API: create, update
and delete events, explicit UTC offsets attached to event times (never the runner's local clock),
true RRULE recurrence, and idempotent deletes (a 404/410 from an already-gone event is success, not
a failure).

**7. Track — `app/db/repository.py`, `app/db/models.py`.** Every outcome lives in Postgres:
`processed_emails` (one row per email — its extraction, its status, its Calendar event id, any
human vote or correction), `pipeline_runs` (one row per scheduled/manual run — counts, timing,
outcome), `oauth_tokens` (the Google refresh token, so it survives Lambda's stateless containers).
Atomic claims (`INSERT ... ON CONFLICT ... WHERE ... RETURNING`, decision 2), a Postgres advisory
transaction lock as a single-flight guard against two runs overlapping (decision 3), a per-day
Gemini budget counter, and a retry cap that gives a transiently-failed email a bounded number of
second chances before it's parked for good.

**8. Review, surface one: the dashboard — `app/main.py`, `app/templates/`.** FastAPI + Jinja2 +
htmx (decision 14): server-rendered pages, no separate JS build. Emails are grouped into
categories (`app/categories.py`: needs review, on your calendar, marked incorrect, failed, marked
correct, in progress, denied/skipped — decision-documented so nothing is in two categories or
none), with clickable range filters on the Action Items list. `/metrics` shows run history, filter
pass rate and Gemini usage.

**9. Review, surface two: the Gmail add-on — `gmail-ui/`.** A Google Workspace Add-on (Apps
Script), written in TypeScript and bundled with `esbuild` into the one plain script Apps Script
runs, pushed with Google's `clasp` CLI. Renders the same data as a `CardService` home card inside
Gmail's side panel. Talks to a small, separate FastAPI app (`app/addon_app.py`, `app/addon_api.py`)
over HTTPS, authenticated by verifying the Google identity token (`ScriptApp.getIdentityToken()`,
an RS256 JWT checked against Google's own public-key endpoint in `app/addon_auth.py`) belongs to
the one allowed email and the expected OAuth client id. A committed schema snapshot
(`gmail-ui/contract/api.schema.json`, generated by `backend/scripts/export_addon_schema.py`) is
checked from both sides in CI, so the API and the add-on's TypeScript types cannot silently drift
apart.

**10. The one shared decision layer — `app/review_actions.py`.** Every action a person can take
(approve, decline, vote correct/incorrect, remove, reschedule, schedule an action item, trash the
source email) is a single function here, called by *both* `app/main.py`'s routes and
`app/addon_api.py`'s routes — so the dashboard and the add-on can never disagree about what an
action does or when it's allowed.

**11. Scheduling and infrastructure — `aws/template.yaml`, `lambda_handler.py`.** An hourly
EventBridge Scheduler invokes one Lambda function (AWS SAM, arm64, Python 3.14), which reads its
secret (Gemini key, database URL) from Secrets Manager and writes its own CloudWatch Logs.
CloudWatch alarms on errors and on "no invocations for 3 hours" both notify an SNS topic by email.
GitHub Actions is kept as a manual/fallback trigger only (`.github/workflows/pipeline.yml`) — never
scheduled at the same time as EventBridge, which would double-spend the shared daily Gemini quota.

**12. CI/CD — `.github/workflows/`.** `tests.yml` runs the backend's pytest suite (against a real
`postgres:` service container, not SQLite) and a separate job for `gmail-ui/` (typecheck, Vitest,
build), on every push. `deploy.yml` deploys the SAM stack through GitHub's OIDC provider — a
short-lived, per-run AWS role assumption, no long-lived AWS access keys stored anywhere.

### Full tech stack

| Layer | Technology |
|---|---|
| Pipeline / backend language | Python 3.14 |
| Web framework | FastAPI 0.141 |
| Templating | Jinja2 3.1, with htmx for partial-page updates (no separate JS build) |
...
| Data validation | Pydantic 2 (LLM response schemas, add-on API request/response models) |
| Date/time parsing | `python-dateutil`, Python's standard `zoneinfo` |
| Google API client | `google-api-python-client`, `google-auth`, `google-auth-oauthlib` |
| LLM client | `google-genai` (Gemini) |
| ASGI server | `uvicorn` |
| Add-on language | TypeScript |
| Add-on runtime | Google Apps Script (V8 runtime), `CardService` |
| Add-on bundler | `esbuild` (IIFE bundle, since Apps Script has no module system) |
| Add-on deploy tool | `@google/clasp` |
| Add-on test runner | Vitest |
| Backend test runner | `pytest`, run against a real Postgres instance (never SQLite — see decision 1) |
| Cloud compute | AWS Lambda (arm64, `AWS::Serverless-2016-10-31` / SAM) |
| Scheduling | AWS EventBridge Scheduler (hourly cron) |
| Secrets | AWS Secrets Manager |
| Logging / alerting | AWS CloudWatch (Logs, Alarms), AWS SNS (email) |
| IAM | Least-privilege execution and scheduler roles, scoped to one function |
| CI/CD | GitHub Actions; AWS deploy through GitHub's OIDC provider (no stored AWS keys) |
| Local dev tooling | Docker (throwaway Postgres, and the Lambda build's compiled-wheel image), `cloudflared` (tunnels the add-on's API for testing, since Apps Script cannot reach `localhost`) |

### APIs used

| API | What it's for | Scope / model | Where |
|---|---|---|---|
| **Gmail API** | Fetch messages, detect calendar invites/notifications, trash a message | `gmail.modify` (write access used only for Trash; the scheduled pipeline itself only ever reads — decision 28) | `app/gmail_client.py` |
| **Google Calendar API** | Create, update, delete events; recurring events | `calendar.events` | `app/calendar_client.py` |
| **Google OAuth2 / identity** | The pipeline's own Gmail+Calendar access; a separate identity token proving who is using the Gmail add-on | Pipeline: `gmail.modify` + `calendar.events`. Add-on: `openid`, `.../auth/userinfo.email`, `.../auth/script.external_request`, `.../auth/gmail.addons.execute` | `app/google_auth.py`; `gmail-ui/appsscript.json`, `app/addon_auth.py` |
| **Google Gemini API** | Structured deadline/action-item extraction from email text | `gemini-3.6-flash`, schema-constrained via Pydantic, capped at 20 requests/day (free tier) | `app/llm_client.py` |

### Data model

Three tables (`app/db/models.py`): `processed_emails` (one row per email: its extraction, status,
Calendar event id, any correction/vote), `pipeline_runs` (one row per scheduled or manual run:
counts, timing, outcome), `oauth_tokens` (the Google refresh token, so it survives Lambda's
stateless containers between invocations). No user table yet — the whole schema assumes one person
(see [Scaling to other users](#scaling-to-other-users-a-plan-not-built) and `SCALING.md`).
